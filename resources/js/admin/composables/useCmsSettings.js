...
function resolveThemeState(settings) {
    ...
    const domMode = typeof document !== 'undefined' ? document.documentElement.getAttribute('data-admin-theme-mode') : null
    const domPalette = typeof document !== 'undefined' ? document.documentElement.getAttribute('data-admin-theme-palette') : null
    const mode = normalizeMode(preference.mode ?? settings.admin_theme_mode ?? domMode ?? 'dark')
    const lightPalette = normalizeLightPalette(preference.lightPalette ?? settings.admin_light_palette ?? (mode === 'light' ? domPalette : null) ?? 'slate')
    const darkPalette = normalizeDarkPalette(preference.darkPalette ?? settings.admin_dark_palette ?? (mode === 'dark' ? domPalette : null) ?? 'midnight')
...
function writeThemePreference(themeState) {
    ...
    window.localStorage.setItem(ADMIN_THEME_STORAGE_KEY, JSON.stringify({
...
function normalizeLightPalette(palette) {
    const value = String(palette ?? '').trim().toLowerCase()
